 ...
 * • 节日计算：内置农历算法数组，支持计算法定节假日、民俗节日、国际节日等倒计时。
...
export default async function (ctx) {
  ...
  const customDays = [1,2,3,4,5,6].map(i => ({
    name: getStr(`EXCLUSIVE_NAME_${i}`, i === 1 ? getStr("EXCLUSIVE_NAME", "结婚纪念日") : ""),
    date: getStr(`EXCLUSIVE_DATE_${i}`, i === 1 ? getStr("EXCLUSIVE_DATE", "09/07") : "")
